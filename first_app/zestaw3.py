...
def zestaw3(request):
    ...
    slownik_mies, prac_czasmiesiac, slownik3 = {}, {}, {}
    ...
    unikalne_id = data['Pracownik'].unique().tolist()


    for key in unikalne_id:
        slownik3[key] = fake.seed_instance(key).name()


    zestawienie1_wybory = [slownik3, slownik_mies, lista_rok, slownik3[unikalne_id[0]], min(lista_mies), min(lista_rok)]
    ...
    def glowne_obliczenie_godzin():
        ...
        for key, value in slownik3.items():
            data = data2
            prac_czasmiesiac[key] = 0
            data = data[(data.Pracownik == value) & (data['Data'].dt.year == datetime(int(zestawienie1_wybory[5]), 1, 1).year) & (data['Data'].dt.month == datetime(2000, int(zestawienie1_wybory[4]), 1).month)]
            data_wejscia, data_wyjscia, dzien, przepracowany_czas = 0, 0, 0, 0
            ...
            for index, row in data.iterrows():
                if row['Zdarzenie'] % 2 == 0:
                    ...
                    if data_wejscia != 0:
                        ...
                        data_wejscia = 0
                        prac_czasmiesiac[key] = przepracowany_czas


        for key, value in prac_czasmiesiac.items():
            prac_czasmiesiac[key] = "{0[0]:.0f}:{0[1]:02.0f}".format(convert_seconds(value))


        return render(request, 'first_app/zestaw3.html', {'data':data,'zestaw1':zestawienie1_wybory, 'dni_miesiaca':dni_miesiaca, 'prac_czasmiesiac':prac_czasmiesiac})
